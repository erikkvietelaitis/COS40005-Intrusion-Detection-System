...
pub struct Authentication {
    ...
    //Everything else is persisteuse crate::LaraCore::*;
//use rand::{seq::SliceRandom, Rng};
use CoreTraits::AnalysisModule;
...
impl AnalysisModule for Authentication {
    ...
    fn get_data(&mut self) -> bool {
        ...
        let btmpdump: &str = linux_bridge::auth.btmp_dump();
        let wtmpdump: &str = linux_bridge::auth.wtmp_dump();
        ...
        let flines: Vec<&str>; //= failtestdata.lines().collect();
        let slines: Vec<&str>; //= successtestdata.lines().collect();
        if wtmplineslen > 0{
        ...
        self.current_data = CurrentData {
            cfips: fips,
            csips: sips,
            lastbtmplen: btmplineslen,
            lastwtmplen: wtmplineslen,
        };
        return true;
    }
        ...
        if self.current_data.cfips.len() > 0 {
            ...
            while i1 < self.current_data.cfips.len() {
                ...
                while i2 < self.pfips.len() {
                    if self.current_data.cfips[i1].ip == self.pfips[i2].ip {
                        ...
                        if self.pfips[i2].num < 3 as &u64{
                            results.push(CoreStruts::Log::new(CoreEnums::LogType::Info,self.module_name.clone(),msg,));
                        } else if self.pfips[i2].num < 10 as &u64 {
                            results.push(CoreStruts::Log::new(CoreEnums::LogType::Warning,self.module_name.clone(),msg,));
                            ...
                            results.push(CoreStruts::Log::new(CoreEnums::LogType::Critical,self.module_name.clone(),msg,));
                        }
                    }
                    i2 = i2 + 1;    
                }
                if !inpfips{
                    ...
                    msg.push_str(&self.current_data.cfips[i1].num.to_string().as_str());
                    msg.push_str("' time(s)");
                    if &self.current_data.cfips[i1].num < 3 as &u64{
                        results.push(CoreStruts::Log::new(CoreEnums::LogType::Info,self.module_name.clone(),msg,));
                    } else if &self.current_data.cfips[i1].num < 10 as &u64 {
                        results.push(CoreStruts::Log::new(CoreEnums::LogType::Warning,self.module_name.clone(),msg,));
                    } else if &self.current_data.cfips[i1].num < 1000 as &u64{
                        results.push(CoreStruts::Log::new(CoreEnums::LogType::Serious,self.module_name.clone(),msg,));
                    } else {
                        results.push(CoreStruts::Log::new(CoreEnums::LogType::Critical,self.module_name.clone(),msg,));
                    }
                    self.pfips.push(self.current_data.cfips[i1])
                }
                i1 = i1 + 1;
            }
        }
        if self.current_data.csips.len() > 0 {
            ...
            while i1 < self.current_data.csips.len() {
                ...
                if !inpsips{
                    ...
                    msg.push_str("' time(s)");
                    results.push(CoreStruts::Log::new(CoreEnums::LogType::Info,self.module_name.clone(),msg,));
                    self.psips.push(self.current_data.csips[i1])
                }
                i1 = i1 + 1;
            }
        }
